...
unsigned char otsuThreshold(unsigned char image[BMP_WIDTH][BMP_HEIGTH][BMP_CHANNELS]){
    ...
    for(int i = 0; i < 256; i++){ //TODO Remove this print function
        printf("%d \n", histogram[i]);
    }
    ...
    for(int i = 0; i < 256; i++){
        ...
        if(weightBackground == 0) continue; //If there isnt any pixels in background there is no reason to calculate threshold

        weightForeground = pixels-weightBackground;
        if(weightForeground == 0) break; //If there are no pixels left in foreground, best threshold should already be found
        ...
        variance = (double)weightBackground*(double)weightForeground*(meanBackround-meanForeground)*(meanBackround-meanForeground);
