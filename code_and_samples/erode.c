...
char rightNeighbor(unsigned char image[BMP_WIDTH][BIT_WIDTH], int x, int y) {
    // the right neighbor can be aligned by bit shifting to the left
    if (y == BIT_WIDTH - 1) {
        // if its the right most edge, add 1 as least significant bit
        return ((image[x][y] << 1) | 0x01);
    }
    return ((image[x][y] << 1) | (image[x][y + 1] >> 7)); 
    // else add the most siginficant bit of the last char
}
